  ...
  methods: {
    updateRank() {
      ...
      const dataFiltered = this.$props.data.filter(record => {
        if (this.$props.filters.form !== "all" && record.user_class[0] !== this.$props.filters.form) return false
        if (this.$props.filters.class !== "all" && record.user_class[1] !== this.$props.filters.class) return false
        if (this.$props.filters.gender !== "all" && record.user_gender !== this.$props.filters.gender) return false
        return true
      })
      ...
      let rank = Object.keys(count).map(user_id => {
        return {
          user_id,
          lap_count: count[user_id],
          average_time: this.$props.data.filter(record => record.user_id === user_id).reduce((acc, cur) => acc + cur.record_time, 0) / count[user_id],
          ...this.$props.data.find(record => record.user_id === user_id)
        }
      }).sort((a, b) => b.lap_count - a.lap_count)

      // group out
      const groupByLap = {}, exist = []
      rank.forEach(user => {
      ...
      for (let key in groupByLap) {
        groupByLap[key] = groupByLap[key].sort((a, b) => a.average_time - b.average_time)
      }
    ...
    <div class="list">
      <div class="stat" v-for="(user, index) in showList" :key="index">
        ...
        <div class="right">
          ...
          <span style="width: 50px">{{ user.user_gender === "M" ? "Man" : "Female" }}</span>
          <span style="width: 150px">Average time: {{ user.average_time.toFixed(2) }}s</span>
        </div>
      </div>
